...
const ModalAddBook = (props: IModalAddBook) => {
  ...
  const handleOk = () => {
    ...
    api
      .post(`book`, {
        ...
        author: { id: authorRef?.id, name: authorRef?.label },
        category: { id: categoryRef?.id, name: categoryRef?.label },
        publisher: { id: publisherRef?.id, name: publisherRef?.label },
      ...
      .then((res) => {
        setDataUri("");
        onClose("Success: Create book successful");
        setReload(!reload);
      })
      .catch((err) => {
        setDataUri("");
  ...
  return (
    <Dialog
      sx={{ "& .MuiDialog-paper": { width: "80%", maxHeight: "600px" } }}
      maxWidth="xl"
      open={open}
    >
      <DialogTitle>Add Book Detail</DialogTitle>
      <DialogContent dividers sx={{ display: "flex", flexWrap: "wrap" }}>
        ...
        {error && (
          <Alert severity="error" sx={{ marginTop: "1rem",margin:"auto"}}>
